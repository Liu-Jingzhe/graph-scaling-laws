...
def train(model, device, loader, optimizer, task_type):
    ...
    for step, batch in enumerate(loader):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            d_energy_sum += d_energy.item()
            loss.backward()
            optimizer.step()    
    train_loss = loss_accum/(step+1)
    d_energy_train = d_energy_sum/(step+1)
    print('Train Loss', train_loss)
    print('Training Dirichlet energy',d_energy_train)
    return train_loss, d_energy_train
...
def eval(model, device, loader, evaluator,task_type):
    ...
    d_energy_eval = d_energy_sum/(step+1)
    print('Eval Dirichlet energy',d_energy_eval)

    return evaluator.eval(input_dict),loss_accum/(step+1),d_energy_eval
...
def main():
    ...
    parser.add_argument('--emb_dim', type=int, default=800,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print('Training...')
        train_loss, d_energy_train=train(model, device, train_loader, optimizer, dataset.task_type)

        print('Evaluating...')
        #train_perf = eval(model, device, train_loader, evaluator)
        valid_perf,val_loss, d_energy_val = eval(model, device, valid_loader, evaluator, dataset.task_type)
        test_perf, test_loss, d_energy_test = eval(model, device, test_loader, evaluator, dataset.task_type)
        ...
        wandb.log({"train loss":train_loss,"val acc": valid_perf, "val loss": val_loss, "test acc": test_perf, "test loss":test_loss,"d_energy_train":d_energy_train,"d_energy_val":d_energy_val,"d_energy_test":d_energy_test})
